...
Mon objectif : Intégrer un SOC à Toulouse d'ici la fin de l'année, mettre mon sens de l'observation et ma réactivité au service de la surveillance de vos infrastructures, afin d'évoluer durablement au sein de l'entreprise en montant en compétences et en responsabilités.`,
      ...
      btl1Desc: `Certification pratique axée sur les opérations défensives avancées, validée par un examen de 24 heures d'investigation en environnement simulé. Le cursus couvre l'ensemble du spectre de la sécurité défensive : analyse approfondie de phishing (extraction d'artefacts avec PhishTool, analyse d'en-têtes, rétro-ingénierie d'URL et pièces jointes), gestion de la Cyber Threat Intelligence (CPO, MISP, frameworks MITRE ATT&CK et Pyramid of Pain) et déploiement de règles de détection.
      Sur le volet investigation, la formation valide mes compétences en Digital Forensics et analyse mémoire (collecte avec KAPE, analyse sous Volatility 3, Autopsy et étude des artefacts Windows/Linux), ainsi qu'en manipulation de SIEM pour la corrélation de logs (Splunk, écriture de règles Sigma, Sysmon). Enfin, le cursus consolide la gestion complète du cycle de réponse aux incidents (de la détection à l'éradication via DeepBlueCLI et Wireshark), couplée à la gestion de cas sur TheHive et la rédaction de rapports d'incident structurés.`,
...
My goal: Join a SOC in Toulouse by the end of the year, put my keen observation skills and responsiveness to work monitoring your infrastructure, and grow within the company over the long term as I take on new skills and responsibilities.`,
      ...
      btl1Desc: `Practical certification focused on advanced defensive operations, validated through a 24-hour incident response exam in a simulated live-network environment. The curriculum covers the full spectrum of Blue Team operations: in-depth phishing analysis (artifact extraction with PhishTool, header analysis, URL and attachment triage), Threat Intelligence application (MISP, MITRE ATT&CK, Pyramid of Pain), and custom detection engineering. On the investigation side, it validates my hands-on technical skills in Digital Forensics and memory analysis (evidence acquisition using KAPE, Volatility 3, Autopsy, and Windows/Linux artifact analysis), alongside SIEM operations and log correlation (Splunk, Sysmon, Sigma rules). Lastly, it covers the end-to-end Incident Response lifecycle (detection to remediation using DeepBlueCLI and Wireshark), integrated with case management in TheHive and formal incident report writing.`,
